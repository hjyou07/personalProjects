...
TEST(Movie, CreateFromRow) {
  ...
  ASSERT_EQ(0, strcmp(m1->actor_list[0], "[u'Tim Robbins'"));
  ASSERT_EQ(0, strcmp(m1->actor_list[1], " u'Morgan Freeman'"));
  ASSERT_EQ(0, strcmp(m1->actor_list[2], " u'Bob Gunton']"));
...
TEST(FileParser, BuildMovieIndex) {
  ...
  // in data/test, there's only two genres, Crime and Action,
  // so only two spots in the hashtable should be taken
  ASSERT_EQ(100, index->num_buckets);
  ASSERT_EQ(2, index->num_elements);
  DestroyIndex(index);
