...
It was seen that the data type int and long uses different registers, as the instruction differed from movl for int to mov1 for long, and eax was used for int and rax for long.
...
There was a whole a lot more going on than what was happening in swap_int.c where they swaped integers in main. I compared swap_int.s to swap.s to see how registers are being used differently.  
In swap_int.s, it's pretty short and doesn't use much of the registers as it directly perform everything within main(), but in swap.s, more registers are being used as the code executes in main, jumps to the swap function, then jumps back to the main to execute the rest.
...
The offset I observed was -1616 and -1336 for the each random initialized values
