...
TEST(DirectoryParser, IndexTheFile_OneFile) {
  ...
  // Coffee is in 2nd row of docId 42 
...
TEST(DirectoryParser, ParseTheFiles) {
  ...
  int num_records = ParseTheFiles(docs, ind);
  printf("number of records from files: %d\n", num_records);
...
TEST(DocumentSet, AddMovieToSet) {
  ...
  HTKeyValue kvp;
  LookupInHashtable(mset->doc_index, doc_id, &kvp);
  //Conditional jump or move depends on uninitialised value?
  /**
  * THIS CAME FROM THE ROW ID I DIDN'T MALLOC, SO THE ROWID WAS "UNINITIALIZED"
  */
  PrintOffsetList((LinkedList)kvp.value);
  // It did print out 16, 15 which is the row numbers we expected

  EXPECT_EQ(DocumentSetContainsDoc(mset, doc_id), 0);
  EXPECT_NE(DocumentSetContainsDoc(mset, 75), 0);

  // Destroy movieSet
  // oh fuck me there's 306 frees on 303 allocs shit again
  DestroyDocumentSet(mset);
}
...
TEST(DocIdMap, Full) {
  ...
  HTKeyValue kvp;
  // uh why do we think that key would be 1..?
  //uint64_t key1 = FNVHash64((unsigned char *)f1, (unsigned int)strlen(f1));
  ...
  HTKeyValue kvp2;
  //uint64_t key2 = FNVHash64((unsigned char *)f2, (unsigned int)strlen(f2));
  LookupInHashtable(docs, 2, &kvp2);
  EXPECT_STREQ((const char*)kvp2.value, "file2");
  
  // adding a case: what if I try to put an already existing filename?
  // but if I'm incrementing the hash by the order I put in, this will never get captured.
//  char *f3 = (char*)(malloc(sizeof(char)*10));
//  strcpy(f3, "file2");
//  PutFileInMap(f3, docs);
//  EXPECT_EQ(NumElemsInHashtable(docs), 2);
//  
//  HTKeyValue kvp3;
//  uint64_t key3 = FNVHash64((unsigned char *)f3, (unsigned int)strlen(f3));
//  int exists = LookupInHashtable(docs, 3, &kvp3);
//  printf(" f3 shouldn't exist as -1: %d\n", exists);
  
  char *out = GetFileFromId(docs, 1);
  EXPECT_STREQ(out, "file1");
  // since I malloced f3 but it didn't get added to the DocIdMap,
  // I need to free it here manually? why is it giving a Invalid free()?
